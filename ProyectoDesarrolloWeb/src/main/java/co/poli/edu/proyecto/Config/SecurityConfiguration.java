...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		 http.cors()
			...
			.authorizeRequests()
			.antMatchers("/api/v1/pasantia/**").hasRole("ADMIN")
			.antMatchers("/api/v1/pasantias/**").hasRole("ADMIN")
			.antMatchers("/api/v1/programa/**").hasRole("ADMIN")
			.antMatchers("/api/v1/programas/**").hasRole("ADMIN")
			.antMatchers(HttpMethod.GET, "/api/v1/programa/**").hasRole("USER")
			.antMatchers(HttpMethod.GET, "/api/v1/pasantia/**").hasRole("USER")
			.antMatchers(HttpMethod.GET, "/api/v1/programa").hasRole("USER")
			.antMatchers(HttpMethod.GET, "/api/v1/pasantia").hasRole("USER")			//auth with HTTP Methods
			/*
